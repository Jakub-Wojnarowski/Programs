...
void Euler(double dt, double alfa)
{
	...
	double* v = new double[czas / dt];
	double* x = new double[czas / dt];
	double* Ek = new double[czas / dt];
	double* Ve = new double[czas / dt];
	double* E = new double[czas / dt];
...
void trapez(double dt, double alfa)
{
	...
	for (int i = 0; i < czas / dt - 1; i++)
	{
		...
		while (f1 > 0.000001 || f2 > 0.000001 || f1 < -0.000001 || f2 < -0.000001)
		{
			...
			d_x = (- (1 + dt * alfa / 2) * f1 - dt / 2 * f2) / ((1 + dt / 2 * alfa + dt / (2 * m) * d2V * dt / 2));
			d_v = (- f2 + dt / (2 * m) * d2V * f1) / ((1 + dt / 2 * alfa + dt / (2 * m) * d2V * dt / 2));
